...
using BinanceAlgo.ConnectDB;
using BinanceAlgo.Errors;
using BinanceAlgo.Interval;
using BinanceAlgo.Model;
using BinanceAlgo.Objects;
...
namespace BinanceAlgo
{
    ...
    public partial class MainWindow : Window
    {
        ...
        public int LINE_FULL_SL { get; set; } = 10;
        ...
        private void OpenOrders()
        {
            try
            {
                ...
                {
                    long_quantity_0 = RoundQuantity(variables.USDT_BET * 2 / variables.PRICE_SYMBOL);
                    ...
                    short_quantity_0 = RoundQuantity(long_quantity_0 * 0.5m);
        ...
        private void StartAlgorithm()
        {
            try
            {
                ...
                if (variables.ONLINE_CHART && variables.START_BET && start)
                {
                    ...
                    {
                        // Short
                        short_quantity_1 = RoundQuantity(long_quantity_0 * 0.6m);
                    ...
                    {
                        // Short
                        short_quantity_2 = RoundQuantity(long_quantity_0 * 0.75m);
                    ...
                    {
                        // Short
                        short_quantity_3 = long_quantity_0;
        ...
        public async void PriceOrder(Socket socket, string symbol, long orderId)
        {
            try
            {
                ...
                {
                    await Task.Run(()=> {
                        price = InfoOrderId(socket, symbol, orderId);
                        Dispatcher.Invoke(()=> {
                            short_price_order_0 = price;
        ...
        private void LINE_FULL_SL_TextChanged(object sender, TextChangedEventArgs e)
        {
            NewLineSL();
            plt.Refresh();
        }
        ...
        private void NewLineTP()
        {
            try
            {
                if (list_ohlc.Count > 0)
                {
                    ...
                    line_tp_1_scatter = plt.Plot.AddScatterLines(line_x, line_tp_1_y, Color.Orange, lineStyle: LineStyle.Dash, label: $"{line_tp_1_y[0]} ({Convert.ToDouble(LINE_TP) / 10} %) - take profit price");
        ...
        private void NewLineSL()
        {
            try
            {
                if (list_ohlc.Count > 0)
                {
                    ...
                    {
                        line_tp_2_y[0] = price_short + (price_short / 1000 * -LINE_FULL_SL);
                    }
                    line_tp_2_y[1] = line_tp_2_y[0];
                    line_tp_2_scatter = plt.Plot.AddScatterLines(line_x, line_tp_2_y, Color.DarkRed, lineStyle: LineStyle.Dash, label: $"{line_tp_2_y[0]} (-{Convert.ToDouble(LINE_FULL_SL) / 10} %) - full stop loss price");
